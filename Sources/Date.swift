...
internal extension Date {
    ...
    static func iso8601Formatter(timeZone: TimeZone? = nil) -> DateFormatter {
        ...
        // The locale and timezone properties must be exactly as follows to have a true,
        // time-zone agnostic (i.e. offset of 00:00 from UTC) ISO stamp.
    ...
    /// A custom for deserializing dates that come from Contentstack
    /// This method first attempts to deserialize ISO8601 internetDateTime with fractional seconds
    /// then falls back to attempt the three other ISO8601 variants that the Contentstack web app
    /// enables for editing date fields.
    /// - parameter decoder: The Decoder used to deserialize JSON from Contentstack.
    /// - throws: Error.unsupportedDateFormat if the date isn't one of the three formats the web app supports
    ///           or the format used by Contentstack `sys` properties.
