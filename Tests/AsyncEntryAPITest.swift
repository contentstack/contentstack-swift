//
//  AsyncEntryAPITest.swift
//  Contentstack
//
//  Created by Jigar Kanani on 21/10/23.
//
...
class AsyncEntryAPITest: XCTestCase {
    ...
    func asyncQueryWhere(_ key: EntryModel.FieldKeys, operation: Query.Operation, then completion: @escaping ((Result<ContentstackResponse<EntryModel>, Error>) -> ())) async {
        await self.getEntryQuery().where(queryableCodingKey: key, operation)
            .asyncFind { (result: Result<ContentstackResponse<EntryModel>, Error>, responseType) in
                completion(result)
        }
    }
    ...
    func test01FindAll_EntryQuery() async {
        ...
        await self.getEntryQuery().asyncFind { (result: Result<ContentstackResponse<EntryModel>, Error>, response: ResponseType) in
            switch result {
    ...
    func test02Find_EntryQuery_whereUIDEquals() async {
        ...
        await self.asyncQueryWhere(.uid, operation: .equals(kEntryUID)) { (result: Result<ContentstackResponse<EntryModel>, Error>) in
            switch result {
            case .success(let contentstackResponse):
    ...
    func test03Find_EntryQuery_whereTitleDNotEquals() async {
        ...
        await self.asyncQueryWhere(.title, operation: .notEquals(kEntryTitle)) { (result: Result<ContentstackResponse<EntryModel>, Error>) in
            switch result {
            case .success(let contentstackResponse):
                for entry in contentstackResponse.items {
    ...
    func test04Find_EntryQuery_whereTitleexists() async {
        ...
        await self.asyncQueryWhere(.title, operation: .exists(true)) { (result: Result<ContentstackResponse<EntryModel>, Error>) in
            switch result {
            case .success(let contentstackResponse):
                XCTAssertEqual(contentstackResponse.items.count, 31)
            case .failure(let error):
    ...
    func test05Find_EntryQuery_whereTitleMatchRegex() async {
        ...
        await self.asyncQueryWhere(.title, operation: .matches("Tr")) { (result: Result<ContentstackResponse<EntryModel>, Error>) in
            switch result {
            case .success(let contentstackResponse):
    ...
    func test06Fetch_Entry_fromUID() async {
        ...
        await self.getEntry(uid: kEntryUID).asyncFetch { (result: Result<EntryModel, Error>, response: ResponseType) in
            switch result {
    ...
    func test07Fetch_EntryQuery_WithIncludeContentType() async {
        ...
        await self.getEntryQuery()
            .include(params: .contentType)
            .asyncFind { (result: Result<ContentstackResponse<EntryModel>, Error>, response: ResponseType) in
                switch result {
                case .success(let contentstackResponse):
    ...
    func test08Fetch_Entry_WithGlobalFields() async {
        ...
        await self.getEntry(uid: kEntryUID)
            .include(params: .globalField)
            .asyncFetch { (result: Result<EntryModel, Error>, response: ResponseType) in
                switch result {
                case .success(let model):
    ...
    func test13Find_EntryQuery_whereTitleIncludes() async {
        ...
        await self.asyncQueryWhere(.title, operation: .includes(titleArray)) { (result: Result<ContentstackResponse<EntryModel>, Error>) in
            switch result {
            case .success(let contentstackResponse):
                XCTAssertEqual(contentstackResponse.items.count, 2)
                for entry in contentstackResponse.items {
    ...
    func test14Find_EntryQuery_whereTitleExclude() async {
        ...
        await self.asyncQueryWhere(.title, operation: .excludes(titleArray)) { (result: Result<ContentstackResponse<EntryModel>, Error>) in
            switch result {
            case .success(let contentstackResponse):
    ...
    func test15Find_EntryQuery_wherelessThan() async {
        ...
        await self.getEntryQuery().where(valueAtKey: "session_time.start_time", .isLessThan(date)).asyncFind { (result: Result<ContentstackResponse<EntryModel>, Error>, response) in
            switch result {
            case .success(let contentstackResponse):
                XCTAssertEqual(contentstackResponse.items.count, 29)
                for entry in contentstackResponse.items {
        ...
        await self.getEntryQuery().where(valueAtKey: "session_id", .isLessThan(id)).asyncFind { (result: Result<ContentstackResponse<EntryModel>, Error>, response) in
            switch result {
            case .success(let contentstackResponse):
    ...
    func test16Find_EntryQuery_wherelessThanEqual() async {
        ...
        await self.getEntryQuery().where(valueAtKey: "session_time.start_time", .isLessThanOrEqual(date)).asyncFind { (result: Result<ContentstackResponse<EntryModel>, Error>, response) in
            switch result {
            case .success(let contentstackResponse):
                XCTAssertEqual(contentstackResponse.items.count, 29)
                for entry in contentstackResponse.items {
        ...
        await self.getEntryQuery().where(valueAtKey: "session_id", .isLessThanOrEqual(id)).asyncFind { (result: Result<ContentstackResponse<EntryModel>, Error>, response) in
            switch result {
            case .success(let contentstackResponse):
    ...
    func test17Find_EntryQuery_whereGreaterThan() async {
        ...
        await self.getEntryQuery().where(valueAtKey: "session_time.start_time", .isGreaterThan(date)).asyncFind { (result: Result<ContentstackResponse<EntryModel>, Error>, response) in
            switch result {
            case .success(let contentstackResponse):
                XCTAssertEqual(contentstackResponse.items.count, 2)
                for entry in contentstackResponse.items {
        ...
        await self.getEntryQuery().where(valueAtKey: "session_id", .isGreaterThan(id)).asyncFind { (result: Result<ContentstackResponse<EntryModel>, Error>, response) in
            switch result {
            case .success(let contentstackResponse):
    ...
    func test18Find_EntryQuery_whereGreaterThanEqual() async {
        ...
        await self.getEntryQuery().where(valueAtKey: "session_time.start_time", .isGreaterThanOrEqual(date)).addValue("val", forHTTPHeaderField: "key").asyncFind { (result: Result<ContentstackResponse<EntryModel>, Error>, response) in
            switch result {
            case .success(let contentstackResponse):
                XCTAssertEqual(contentstackResponse.items.count, 2)
                for entry in contentstackResponse.items {
        ...
        await self.getEntryQuery().where(valueAtKey: "session_id", .isGreaterThanOrEqual(id)).asyncFind { (result: Result<ContentstackResponse<EntryModel>, Error>, response) in
            switch result {
            case .success(let contentstackResponse):
    ...
    func test19Find_EntryQuery_OrderBySessionTime() async {
        ...
        await self.getEntryQuery()
            ...
            .asyncFind { (result: Result<ContentstackResponse<EntryModel>, Error>, response) in
            switch result {
            case .success(let contentstackResponse):
                var date: Date?
                for entry in contentstackResponse.items {
    ...
    func test27Fetch_Entry_IncludeReference() async {
        ...
        await self.getEntry(uid: kEntryUID)
            .includeReference(with: ["track", "room"])
            .asyncFetch { (result: Result<EntryModel, Error>, response) in
            switch result {
            case .success(let model):
    ...
    func test32Fetch_EntryQuery_WithoutFallback_Result() async {
        ...
        await self.getEntryQuery().locale(locale)
            .asyncFind { (result: Result<ContentstackResponse<EntryModel>, Error>, response: ResponseType) in
                switch result {
    ...
    func test34Fetch_Entry_UIDWithoutFallback_NoResult() async {
        ...
        await self.getEntry(uid: kEntryLocaliseUID)
            .locale("en-gb")
            .asyncFetch { (result: Result<EntryModel, Error>, response: ResponseType) in
            switch result {
            case .success(let model):
